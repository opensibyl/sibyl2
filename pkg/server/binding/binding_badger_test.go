...
func TestBadgerFunc(t *testing.T) {
	...
	assert.Nil(t, err)
	err = d.CreateFuncTag(wc, f.GetSignature(), newTag2, ctx)
	assert.Nil(t, err)
	...
	assert.Nil(t, err)
	assert.Len(t, fs, 1)
	fs, err = d.ReadFunctionsWithTag(wc, newTag+";"+newTag2, ctx)
	assert.Nil(t, err)
	assert.Len(t, fs, 1)
	fs, err = d.ReadFunctionsWithTag(wc, notExistedTag, ctx)
	assert.Nil(t, err)
	assert.Len(t, fs, 0)
}
