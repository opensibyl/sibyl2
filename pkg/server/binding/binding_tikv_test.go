...
func TestTikvFunc(t *testing.T) {
	...
	newTag2 := "THIS_IS_ANOTHER_TAG"
	notExistedTag := "NOT_EXISTED_TAG"
	err = tikvTestDriver.CreateFuncTag(wc, f.GetSignature(), newTag, ctx)
	assert.Nil(t, err)
	err = tikvTestDriver.CreateFuncTag(wc, f.GetSignature(), newTag2, ctx)
	assert.Nil(t, err)

	newF, err := tikvTestDriver.ReadFunctionWithSignature(wc, f.GetSignature(), ctx)
	assert.Nil(t, err)
	assert.Len(t, newF.Tags, 2)
	fs, err := tikvTestDriver.ReadFunctionsWithTag(wc, newTag, ctx)
	assert.Nil(t, err)
	assert.Len(t, fs, 1)
	fs, err = tikvTestDriver.ReadFunctionsWithTag(wc, newTag+";"+newTag2, ctx)
	assert.Nil(t, err)
	assert.Len(t, fs, 1)
	fs, err = tikvTestDriver.ReadFunctionsWithTag(wc, notExistedTag, ctx)
	assert.Nil(t, err)
	assert.Len(t, fs, 0)
}
...
func TestTikvFuncCtx(t *testing.T) {
	...
	err = tikvTestDriver.CreateFuncFile(wc, called, ctx)
	assert.Nil(t, err)
